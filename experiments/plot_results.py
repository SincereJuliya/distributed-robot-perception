...
-------------------------------
Produce report-ready figures from the CSVs written by exp1..exp4.
...
core.run_one_cycle now stores explicitly.

Outputs (130 dpi PNG):
  fig1_error_scatter.png      — (err_x, err_y) scatter with ρ_m circle
  fig1_error_histogram.png    — histogram of ‖e‖ for exp1
  fig2_radius_sweep.png       — error / λ₂ / time vs r_c with error bars
...
def fig_error_scatter(rows, out_path, rho_m: float = 50):
    ...
    ax.plot(rho_m * np.cos(th), rho_m * np.sin(th),
            "r--", lw=1.5, label=f"ρ_m = {rho_m:.0f} px")
...
def fig_error_histogram(rows, out_path):
    ...
    ax.axvline(50, color="red", ls=":", lw=1.5, label="ρ_m = 50 px")
...
def fig_radius_sweep(rows, out_path):
    ...
    err_pt, err_lo, err_hi = [], [], []
    tot_pt, tot_lo, tot_hi = [], [], []
    lam_pt, lam_lo, lam_hi = [], [], []
    zero_mean = []
    for rc in radii:
        sub  = [r for r in rows
                if int(r["comm_radius"]) == rc and r["completed"]]
        p, lo, hi = bootstrap_ci([r["err"]         for r in sub]); err_pt.append(p); err_lo.append(p-lo); err_hi.append(hi-p)
        p, lo, hi = bootstrap_ci([r["total_steps"] for r in sub]); tot_pt.append(p); tot_lo.append(p-lo); tot_hi.append(hi-p)
        p, lo, hi = bootstrap_ci([r["lam2_mean"]   for r in sub]); lam_pt.append(p); lam_lo.append(p-lo); lam_hi.append(hi-p)
        zero_mean.append(np.mean([r["lam2_zero_pct"] for r in sub]))

    fig, axes = plt.subplots(1, 3, figsize=(13, 4), facecolor="white")
    axes[0].errorbar(radii, err_pt, yerr=[err_lo, err_hi],
                     fmt="o-", color="#3b7a8c", capsize=4)
    axes[0].axhline(50, color="red", ls="--", lw=1, label="ρ_m = 50 px")
    axes[0].set_xlabel("r_c [px]"); axes[0].set_ylabel("Localization error [px]")
    axes[0].set_title("Localization error (95% CI)")
    axes[0].legend(); axes[0].grid(True, alpha=0.3)

    ax2 = axes[1]
    ax2.errorbar(radii, lam_pt, yerr=[lam_lo, lam_hi],
                 fmt="s-", color="#c89000", capsize=4, label="mean λ₂")
    ax2b = ax2.twinx()
    ax2b.plot(radii, zero_mean, "v--", color="#b04040", label="% disconnected")
    ax2.set_xlabel("r_c [px]"); ax2.set_ylabel("Mean λ₂")
    ax2b.set_ylabel("% steps with λ₂ = 0")
    ax2.set_title("Connectivity"); ax2.grid(True, alpha=0.3)
    ax2.legend(loc="upper left"); ax2b.legend(loc="upper right")

    axes[2].errorbar(radii, tot_pt, yerr=[tot_lo, tot_hi],
                     fmt="^-", color="#5a8b5a", capsize=4)
    axes[2].set_xlabel("r_c [px]"); axes[2].set_ylabel("Steps from leak to report")
    axes[2].set_title("Total time (95% CI)"); axes[2].grid(True, alpha=0.3)
...
def fig_explorers_box(rows, out_path):
    ...
    axes[0].boxplot([L, R], labels=["Lloyd", "RandomWalk"],
                    patch_artist=True,
    ...
    axes[1].boxplot([LT, RT], labels=["Lloyd", "RandomWalk"],
                    patch_artist=True,
