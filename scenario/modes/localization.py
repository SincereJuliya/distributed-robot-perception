...
Phases:
  ...
  CONSENSUS → DKF beliefs fuse; σ falls below threshold (stable)
...
class LocalisationMode:
    ...
    def __init__(self, robots, gas, lloyd, dkf, free_space, event_log):
        ...
        self.agreed_px              = None
        self.consensus_history      = []    # σ over time
        self.lambda2_history        = []    # λ₂ over time
        self.leak_count             = 0
    ...
    def update(self, step):
        ...
        if ph == Phase.MONITOR:
            ...
            # Each robot continues its Lloyd-Voronoi patrol until its OWN
            # local sensor reads above the detection threshold.
            ...
            # Any robot with has_detected=True (real peak seen in radius)
            # serves as a reliable scout. Its POSITION is always accurate
            # (GPS), even if its DKF belief is biased (e.g. degraded robot).
            # Nearby robots break their Lloyd patrol and head to the
            # scout's position to VERIFY with their own clean sensors.
            #
            # We deliberately use has_detected (not self_observed) because
            # self_observed can fire from sensor noise far from gas;
            # has_detected requires a real peak in the sensing radius.
            ...
            for r in alive_robots:
                if r.has_detected and r.dkf_mu is not None:
                    if r.manual_degradation:
                        ...
                        # Don't follow biased belief — clean robots need
                        # a stable point to converge to.
                    ...
                    # Head toward the nearest scout's POSITION (reliable)
                    # within comm range.
                    nearest = None
                    nearest_d = float("inf")
                    for s in scouts:
            ...
            # Transition to CONSENSUS only when MAJORITY of robots have made
            # their OWN observation by physically being near the gas.
            alive = alive_robots
            ...
            # Log detection only when has_detected count changes
            # OR every 60 steps as heartbeat. We log ONLY has_detected
            # (real peak in sensor radius), not self_observed (which
            # triggers from sensor noise even far from the gas — useless
            ...
            if (count_changed or step % 60 == 0) and (n_sniff > 0 or n_detect > 0):
                ...
                self.log.info(step, "DETECT",
                    f"{n_detect}/{threshold} located the leak  λ₂={lam2:.2f}",
                    severity="warn")
            ...
            self.gas.stop_all()
            self.lloyd.update_targets_uniform()
            # Stay in REPORTED for a generous window so the user can
            # SEE the robots return to their Voronoi cells and resume
            # patrolling. Exit only when (a) gas has cleared AND (b) at
            # least REPORT_HOLD_STEPS have elapsed since entering REPORTED.
    ...
    def _start_leak(self, step):
        ...
        # CRITICAL: clear stale DKF beliefs from any previous cycle
        # (otherwise consensus instantly "locks" on the old leak location)
        for r in self.robots:
            r.dkf_mu = None; r.dkf_P = None
            r.dkf_seed_mu = None
            r.has_detected = False
    ...
    def _consensus_step(self, step):
        # Apply the same scout-attraction logic as in LEAK phase:
        # robots without their own belief should head toward a scout
        # (within or out of comm range — once in CONSENSUS we use the
        # broadest possible attraction so isolated robots reconnect).
        alive_robots = [r for r in self.robots if r.is_alive]
        scouts       = [r for r in alive_robots if r.has_detected]
        # Centroid of all scouts — used as fallback for robots out of
        # comm range of any individual scout
        if scouts:
        ...
        for r in alive_robots:
            if r.dkf_mu is not None:
                ...
                # No own belief but there are scouts — head toward
                # nearest scout if within comm range, else toward
                # the scout centroid to break out of isolation.
                nearest = None
                nearest_d = float("inf")
                for s in scouts:
                    if s.id == r.id: continue
        ...
        # σ MUST be computed from SEED beliefs (the initial estimate
        # each robot made from its OWN sensor), not from current
        # dkf_mu. Reason: gossip runs 18 fusion rounds per sim.step()
        # BEFORE we get here, so current dkf_mu values have already
        # been merged toward one another → estimate_disagreement()
        # would return ≈0 immediately. The seed beliefs preserve each
        # robot's independent observation and give the true picture of
        # initial disagreement, which only shrinks as more robots reach
        # the source.
        sigma_display = self.dkf.estimate_disagreement_selfobs()
        # Fallback: if not enough healthy seeds yet (e.g., when
        # multiple degraded robots dominate the observers), fall back
        # to the spread of ALL current beliefs so we at least show a
        # number in the panel.
        ...
        alive = alive_robots
        # Count healthy robots with belief — that's what really matters
        # for a CORRECT consensus
    ...
    def _reset_for_next_cycle(self):
        ...
        for r in self.robots:
            ...
            r.has_detected = False
            r.self_observed = False
            # Clear patrol waypoint so Lloyd picks a fresh one
            # inside the (new) Voronoi cell
