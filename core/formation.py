...
Each robot is assigned a slot offset delta_i from the formation center, and runs
a Laplacian consensus law using only its neighbours within communication range:

    u_i = -k_f * sum_{j in N_i} [(p_i - delta_i) - (p_j - delta_j)] - k_c (p_i - delta_i - c)

The first term is consensus on the formation error variables e_i = p_i - delta_i;
the second anchors the team to the leader path c. u_i is saturated to v_max and
applied through the robot's own dynamics via r.target + r.move().
"""
...
class FormationControl:
    ...
    def step_center(self):
        ...
        if self.shape == "circle":
            self.rotation += config.FORMATION_ROT_RATE
        else:
            self.rotation = 0.0
        ...
        if path == "circle":
            ...
            # Pick a new waypoint either when the timer expires OR when we arrive close to it
    ...
    def _raw_offset(self, slot, n_total):
        ...
        if self.shape == "triangle":
            ...
            r = config.FORMATION_RADIUS
            # 3 vertices at 90 deg, 210 deg, 330 deg (pointing up)
    ...
    def update_targets(self):
        alive = [r for r in self.robots if r.is_alive]
        n = len(alive)
        # Always reassign slots to a contiguous 0..n-1 range, so that a death anywhere in the team is absorbed and the formation is restored automatically
        self.assign_slots()

        # Consensus-based formation law (Olfati-Saber, Fax & Murray 2007).
        # Each robot computes u_i from its OWN state and the state of the
        # neighbours j within communication range r_c only:
        #
        #   e_i = p_i - delta_i                        (formation error variable)
        #   u_i = -k_f * sum_{j in N_i} (e_i - e_j)    (Laplacian consensus term)
        #         -k_c * (e_i - c)                     (anchor to the leader path)
        #
        # Stacked over the team the first term is -k_f (L kron I_2) e, so the
        # disagreement contracts at the rate set by lambda_2(L), matching Eq. (2).
        kf = config.FORMATION_GAIN
        kc = config.FORMATION_ANCHOR_GAIN

        # Frozen snapshot so every u_i uses the same round of messages
        state = {}
        for r in alive:
            state[r.id] = (r.position.copy(),
                           self.slot_offset(r.formation_slot, n))
        ...
        for r in alive:
            ...
            # consensus term over the communication neighbourhood
            for q in r.neighbours(self.robots):
            ...
            # anchor term
            u += -kc * (e_i - self.center)

            # saturate to v_max (Eq. 1)
            nu = np.linalg.norm(u)
            if nu > r.speed:
                u = u * (r.speed / nu)

            r.target = p_i + u          # integrated by the robot's own move()
            r._smooth_centroid = None   # clean switch back to Lloyd later
...
# Each robot is assigned a slot in a formation defined by a shape function that maps slot_index -> offset relative to the formation center:

#     target_i = formation_center + offset(slot_i, shape)

# Positions are updated by a consensus law p_i <- (1-k) p_i + k (center+offset_i) with k = FORMATION_GAIN
# """
...
#         # Consensus-based formation law 
#         #   p_i <- (1 - k) p_i + k (center + offset_i),  k = FORMATION_GAIN
#         # Position is driven directly by the consensus update
#         k = config.FORMATION_GAIN
#         for r in alive:
#             offset = self.slot_offset(r.formation_slot, n)
#             desired = self.center + offset
#             r.position = (1.0 - k) * r.position + k * desired
#             r.target = desired.copy()   # for visualisation / formation_error
#             r._smooth_centroid = None   # clean switch back to Lloyd later
