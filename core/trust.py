...
Each robot maintains a trust score for every other robot:
    trust_in_others[other_id] in [TRUST_MIN, TRUST_MAX]

Update rule (each step where both robots have a DKF belief):
    disagreement_ij = ||mu_i - mu_j||
    target_trust    = exp(-disagreement_ij / TRUST_DISAGREE_PX)
    new_trust       = (1 - alpha) * old_trust + alpha * target_trust
...
class TrustReputation:
    ...
    This class is the *update rule* that maintains those scores based on observed disagreement.
    """
    ...
    def update_step(self):
        """
        Update trust scores from OBSERVABLE signals only. 
