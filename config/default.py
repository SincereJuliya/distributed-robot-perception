...
config/default.py
-----------------
All tunable parameters in one place. Edit values; do NOT add logic here.
"""
...
# Optional obstacles (white rectangles, like Kapoutsis et al.)
USE_OBSTACLES = False
...
# (analytical Eq.2 of Facinelli/Fontanelli 2019, more physically accurate
# but asymmetric — peak is downwind of the source, like in the paper)
PLUME_MODEL = "diffusion"
# Atmospheric stability class (A=most unstable, F=stable). "D" = neutral.
PASQUILL_STABILITY = "D"
# Gaussian-plume stack parameters (matching Facinelli et al. 2019 § V)
STACK_HEIGHT_M  = 3.0       # H_s — stack height in metres
STACK_EMISSION  = 5.0       # Q — emission rate (g/s) — paper § V
...
# ── Sensor model — MiniPID 2 by Ion Science (paper § II-A) ───────────────────
...
MINIPID2_SENSITIVITY_PPB    = 5.0    # rated sensitivity (datasheet)
MINIPID2_NOISE_FLOOR_PPB    = 10.0   # 1-sigma noise floor (datasheet)
MINIPID2_RANGE_M            = 7.0    # effective range (~70 px)
# Noise std σ(d) = σ0 · (1 + (d/ρ)²)   — eq.(3-4) of paper
...
COMM_RADIUS     = 500       # px ≈ 50 m — typical for WiFi/radio at this scale.
                            # Calibrated so the network is connected
                            # during patrol most of the time (λ₂ > 0)
                            # while still allowing occasional disconnects
                            # when a robot strays into a far corner.
...
# Random walk mirrors § III-B of Facinelli et al. 2019. Allows direct
# benchmarking of Lloyd vs Random Walk in batch experiments.
...
# Muted robot palette (designed for white background)
PALETTE = [
    "#3b7a8c",  # teal
    "#b04040",  # muted red
    "#7a5a8c",  # purple
